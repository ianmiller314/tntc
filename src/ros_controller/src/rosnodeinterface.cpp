...
// C/C++ source code generated on : Thu Oct 24 12:30:40 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ros_controller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ros_controller_M, (NULL));
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ros_controller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ros_controller_M)
                == (NULL)) || rtmGetStopRequested(ros_controller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ros_controller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
